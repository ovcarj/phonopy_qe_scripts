...
The data required for plotting will be parsed from ``pwscf.out``, ``pp_bands.out``, ``explicit_labels.pkl`` and the ``*dat.gnu`` file generated by QE. The image will be saved to ``bands.png`` and all the data required for plotting will be saved to ``bands.pkl``.
